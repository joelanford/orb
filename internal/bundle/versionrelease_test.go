...
func TestNewVersionRelease(t *testing.T) {
	type testCase struct {
		...
		release   string
		expectErr bool
		expectVR  *bundle.VersionRelease
	}
	for _, tc := range []testCase{
		{
			name:      "valid version and release",
			version:   "1.2.3",
			release:   "4",
			expectErr: false,
			expectVR: &bundle.VersionRelease{
				Version: bsemver.MustParse("1.2.3"),
				Release: bundle.Release([]bsemver.PRVersion{{VersionNum: 4, IsNum: true}}),
			},
		},
		{
			name:      "valid version with multi-segment release",
			version:   "1.0.0",
			release:   "2.el9",
			expectErr: false,
			expectVR: &bundle.VersionRelease{
		...
		t.Run(tc.name, func(t *testing.T) {
			actual, err := bundle.NewVersionRelease(tc.version, tc.release)
			if tc.expectErr {
				assert.Error(t, err)
				assert.Nil(t, actual)
			} else {
				require.NoError(t, err)
				assert.Equal(t, tc.expectVR, actual)
			}
		})
	}
}
