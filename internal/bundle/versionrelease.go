...
func NewVersionRelease(vStr, rStr string) (*VersionRelease, error) {
	vers, err := bsemver.Parse(vStr)
	if err != nil {
		return nil, err
	}

	rel, err := NewRelease(rStr)
	if err != nil {
		return nil, err
	}
...
func (vr *VersionRelease) AsLegacyRegistryV1Version() bsemver.Version {
	...
	if len(vr.Release) > 0 {
		v.Build = lo.Map(vr.Release, func(i bsemver.PRVersion, _ int) string { return i.String() })
	}
	return v
}
...
func (r Release) String() string {
	rStrs := lo.Map(r, func(i bsemver.PRVersion, _ int) string { return i.String() })
	return strings.Join(rStrs, ".")
}

func (r *Release) UnmarshalJSON(bytes []byte) error {
	var rStr string
	if err := json.Unmarshal(bytes, &rStr); err != nil {
		fmt.Println(string(bytes))
		return fmt.Errorf("foobar: %w", err)
	}
	var err error
	*r, err = NewRelease(rStr)
	if err != nil {
		return err
	}
	return nil
