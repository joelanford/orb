...
// FromFS loads a registry+v1 bundle from a filesystem. The filesystem is expected to
// conform to the registry+v1 format:
//
//	metadata/annotations.yaml
//	metadata/properties.yaml (optional)
//	manifests/
//	  - csv.yaml
//	  - ...
//
// The manifests directory must not contain subdirectories.
func FromFS(fsys fs.FS) (RegistryV1, error) {
	reg := RegistryV1{}

	annotationsFileData, err := fs.ReadFile(fsys, filepath.Join("metadata", "annotations.yaml"))
	if err != nil {
		return reg, err
	}
	...
	foundCSV := false
	if err := fs.WalkDir(fsys, manifestsDir, func(path string, e fs.DirEntry, err error) error {
		if err != nil {
			return err
		}
		if e.IsDir() {
		...
		manifestFile, err := fsys.Open(path)
		if err != nil {
			return err
		}
	...
	if err := copyMetadataPropertiesToCSV(&reg.CSV, fsys); err != nil {
		return reg, err
	}
...
// copyMetadataPropertiesToCSV copies properties from metadata/properties.yaml into
// the CSV's .metadata.annotations["olm.properties"] value, preserving any properties
// that are already present in the annotations.
func copyMetadataPropertiesToCSV(csv *v1alpha1.ClusterServiceVersion, fsys fs.FS) error {
	...
	if csv.Annotations == nil {
		csv.Annotations = make(map[string]string)
	}
