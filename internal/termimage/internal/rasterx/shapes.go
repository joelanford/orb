...
func AddArc(points []float64, cx, cy, px, py float64, p Adder) (lx, ly float64) {
	...
	// Normalize deltaEta to [-pi, pi] to get the shortest arc direction.
	// FindEllipseCenter already selected the correct center based on the
	// large-arc flag, so the shortest arc from the center's perspective
	// is always the correct one.
	if deltaEta > math.Pi {
		deltaEta -= 2 * math.Pi
	} else if deltaEta < -math.Pi {
		deltaEta += 2 * math.Pi
	}

	// Adjust direction based on sweep flag per the W3C SVG spec:
	// fS=1 (sweep=true) requires deltaEta > 0 (counter-clockwise)
	// fS=0 (sweep=false) requires deltaEta < 0 (clockwise)
	if deltaEta < 0 && sweep {
		deltaEta += math.Pi * 2
